...
std::vector<It_coeff_struct> It_coeff(std::vector<Species>& species_list, std::vector<Roots>& species_roots, double b_value ){
	...
	struct It_coeff_struct It_coeff;
	...
	for (i = 0 ; i < species_roots.size() ; i++){
			...
			if (species_roots[i].real_root != 0){
    		    ...
    		    coshype_term = sigma * l_2pi_over_b_m_sigma_r * coshype_term;
    		    sinhype_term =  l_2pi_over_b_m_sigma_r * l_2pi_over_b_m_sigma_r * sinhype_term;
    		    It_coeff.lambda1[ii] = (gamma_u - l_2pi_kappa_sqrd);
    		    It_coeff.lambda2[ii] =  denominator_x * (b*l_2pi_over_b_m_sigma_r - coshype_term - sinhype_term);
				ii++;
    		}
    		else{
    		    ii = 0;
    		...
    		for (j = 0 ; j < max_num_root ; j++){
        		...
        		cosine_term = sigma * q_2pi_over_b_m_sigma_r * cosine_term;
        		sine_term =  q_2pi_over_b_m_sigma_r * q_2pi_over_b_m_sigma_r * sine_term;
        		It_coeff.lambda1[ii] = (gamma_u + q_2pi_kappa_sqrd);
        		It_coeff.lambda2[ii] = denominator_y * (-b*q_2pi_over_b_m_sigma_r + cosine_term + sine_term);
                
        		ii++;
        	}
        It_coeff_arr.push_back(It_coeff);
	}
